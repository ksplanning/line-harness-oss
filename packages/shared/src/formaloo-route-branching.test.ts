...
describe('R-1 — 後方互換: jump 未使用 + 既存 show/hide + form_type 未設定 が byte/挙動不変', () => {
  test('compound-fix: 同一 source の show/hide は 1 item にまとめ actions 配列に格納 (別々 top-level item にしない)', () => {
    ...
    // 同一 source → 1 top-level item・actions 配列に 2 件 (Formaloo が 2 番目以降を無視する誤動作を防ぐ)。
    expect(out).toEqual([
      {
        type: 'field', identifier: 'src',
        actions: [
          { action: 'show', args: [{ type: 'field', identifier: 'tgt' }], when: { operation: 'is', args: [{ type: 'field', value: 'src' }, { type: 'constant', value: 'はい' }] } },
          { action: 'hide', args: [{ type: 'field', identifier: 'tgt2' }], when: { operation: 'is_not', args: [{ type: 'field', value: 'src' }, { type: 'constant', value: 'いいえ' }] } },
        ],
      },
    ])
  })
...
describe('compound-fix (closer O-1 実機再現) — 同一 source 複数 jump を 1 item にグルーピング + pull 展開', () => {
  const abcRules: HarnessLogicRule[] = [
  ...
  test('A/B/C 3 jump ルール → 1 top-level item・actions 配列に 3 件 (別々 item にしない=誤動作防止)', () => {
    const out = toFormalooRawLogic(abcRules, slugId, () => textField('q1')) as any[]
    expect(out).toHaveLength(1)
    expect(out[0].identifier).toBe('q1')
    expect(out[0].actions).toHaveLength(3)
    expect(out[0].actions.map((a: any) => a.action)).toEqual(['jump', 'jump', 'jump'])
    expect(out[0].actions.map((a: any) => a.args[0].identifier)).toEqual(['pA', 'pB', 'pC'])
    expect(out[0].actions.map((a: any) => a.when.args[1].value)).toEqual(['A', 'B', 'C'])
  })

  test('単一 jump ルールは従来どおり 1 item・1 action (byte 一致・回帰なし)', () => {
    const out = toFormalooRawLogic([abcRules[0]], slugId, () => textField('q1')) as any[]
    expect(out).toHaveLength(1)
    expect(out[0].actions).toHaveLength(1)
  })

  test('異なる source のルールは別々の item (source 単位グルーピング)', () => {
    const rules: HarnessLogicRule[] = [
      { id: 'r1', sourceFieldId: 'q1', operator: 'equals', value: 'A', action: 'jump', targetFieldId: 'pA' },
      { id: 'r2', sourceFieldId: 'q2', operator: 'equals', value: 'X', action: 'jump', targetFieldId: 'pX' },
    ]
    const out = toFormalooRawLogic(rules, slugId, () => textField('s')) as any[]
    expect(out).toHaveLength(2)
    expect(out.map((i) => i.identifier)).toEqual(['q1', 'q2'])
  })

  test('pull: compound multi-jump item を N 本の flat jump rule に展開 (対称往復)', () => {
    const raw = toFormalooRawLogic(abcRules, slugId, () => textField('q1'))
    const back = fromFormalooRawLogic(raw, identity)
    expect(back).toHaveLength(3)
    expect(back.map((r) => r.action)).toEqual(['jump', 'jump', 'jump'])
    expect(back.map((r) => r.value)).toEqual(['A', 'B', 'C'])
    expect(back.map((r) => r.targetFieldId)).toEqual(['pA', 'pB', 'pC'])
    expect(back.map((r) => r.sourceFieldId)).toEqual(['q1', 'q1', 'q1'])
    // 展開した flat rule には additive を付けない (各 action = 独立単一条件)
    expect(back.every((r) => r.conditions === undefined && r.actions === undefined && r.raw === undefined)).toBe(true)
  })

  test('is_not jump も逐語展開される', () => {
    const raw = toFormalooRawLogic([
      { id: 'r1', sourceFieldId: 'q1', operator: 'not_equals', value: 'A', action: 'jump', targetFieldId: 'pA' },
      { id: 'r2', sourceFieldId: 'q1', operator: 'equals', value: 'B', action: 'jump', targetFieldId: 'pB' },
    ], slugId, () => textField('q1'))
    const back = fromFormalooRawLogic(raw, identity)
    expect(back.map((r) => r.operator)).toEqual(['not_equals', 'equals'])
  })

  test('isExpandableMultiJumpItem: 全 action jump+単一 leaf when=true / show/hide 混在や AND/OR は false', () => {
    const allJump = (toFormalooRawLogic(abcRules, slugId, () => textField('q1')) as any[])[0]
    expect(isExpandableMultiJumpItem(allJump)).toBe(true)
    // show/hide 複数アクション (matrix item2 相当) は展開しない
    const showHide = { type: 'field', identifier: 'q1', actions: [
      { action: 'show', args: [{ type: 'field', identifier: 't1' }], when: { operation: 'is', args: [{ type: 'field', value: 'q1' }, { type: 'choice', value: 'a' }] } },
      { action: 'hide', args: [{ type: 'field', identifier: 't2' }], when: { operation: 'is', args: [{ type: 'field', value: 'q1' }, { type: 'choice', value: 'b' }] } },
    ] }
    expect(isExpandableMultiJumpItem(showHide)).toBe(false)
    // AND 結合の jump は展開しない
    const andJump = { type: 'field', identifier: 'q1', actions: [
      { action: 'jump', args: [{ type: 'field', identifier: 'p1' }], when: { operation: 'and', args: [] } },
      { action: 'jump', args: [{ type: 'field', identifier: 'p2' }], when: { operation: 'is', args: [{ type: 'field', value: 'q1' }, { type: 'choice', value: 'b' }] } },
    ] }
    expect(isExpandableMultiJumpItem(andJump)).toBe(false)
    // 単一 action は false (従来経路)
    const single = (toFormalooRawLogic([abcRules[0]], slugId, () => textField('q1')) as any[])[0]
    expect(isExpandableMultiJumpItem(single)).toBe(false)
  })

  test('compound multi-jump は weakened として数えない (欠けなく展開されるため)', () => {
    const raw = toFormalooRawLogic(abcRules, slugId, () => textField('q1'))
    expect(countWeakenedFormalooRules(raw)).toBe(0)
  })
})
