...
const ACTIVE_SELECT = `
  SELECT id, line_account_id, form_id, spreadsheet_id, sheet_name,
         sync_direction, conflict_policy, is_active, created_at, updated_at
...
export async function getSheetsConnection(db: D1Database, id: string): Promise<SheetsConnection | null> {
  const row = await db.prepare(`${ACTIVE_SELECT} AND id = ?`).bind(id).first<SheetsConnectionRow>();
...
export async function createSheetsConnection(
  ...
  const created = await getSheetsConnection(db, id);
...
export async function updateSheetsConnection(
  ...
  const result = await db.prepare(
    `UPDATE sheets_connections
     SET spreadsheet_id = ?, sheet_name = ?, sync_direction = ?, updated_at = ?
     WHERE id = ? AND is_active = 1 AND deleted_at IS NULL`,
  ).bind(input.spreadsheetId, input.sheetName, input.syncDirection, jstNow(), id).run();
  if ((result.meta.changes ?? 0) !== 1) return null;
  return getSheetsConnection(db, id);
}

export async function softDeleteSheetsConnection(db: D1Database, id: string): Promise<boolean> {
  const now = jstNow();
  const result = await db.prepare(
    `UPDATE sheets_connections
