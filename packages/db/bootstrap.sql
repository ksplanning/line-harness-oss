...
CREATE TRIGGER trg_rich_menu_rule_friend_update AFTER UPDATE OF metadata, line_account_id ON friends WHEN OLD.metadata IS NOT NEW.metadata OR OLD.line_account_id IS NOT NEW.line_account_id BEGIN INSERT INTO rich_menu_rule_evaluation_queue (friend_id, attempts, available_at, last_error, updated_at) VALUES (NEW.id, 0, strftime('%Y-%m-%dT%H:%M:%f', 'now', '+9 hours'), NULL, strftime('%Y-%m-%dT%H:%M:%f', 'now', '+9 hours')) ON CONFLICT(friend_id) DO UPDATE SET attempts = 0, available_at = excluded.available_at, last_error = NULL, updated_at = excluded.updated_at; END;

CREATE TRIGGER trg_rich_menu_rule_tag_delete AFTER DELETE ON friend_tags BEGIN INSERT INTO rich_menu_rule_evaluation_queue (friend_id, attempts, available_at, last_error, updated_at) SELECT OLD.friend_id, 0, strftime('%Y-%m-%dT%H:%M:%f', 'now', '+9 hours'), NULL, strftime('%Y-%m-%dT%H:%M:%f', 'now', '+9 hours') WHERE EXISTS (SELECT 1 FROM friends WHERE id = OLD.friend_id) ON CONFLICT(friend_id) DO UPDATE SET attempts = 0, available_at = excluded.available_at, last_error = NULL, updated_at = excluded.updated_at; END;

CREATE TRIGGER trg_rich_menu_rule_tag_insert AFTER INSERT ON friend_tags BEGIN INSERT INTO rich_menu_rule_evaluation_queue (friend_id, attempts, available_at, last_error, updated_at) VALUES (NEW.friend_id, 0, strftime('%Y-%m-%dT%H:%M:%f', 'now', '+9 hours'), NULL, strftime('%Y-%m-%dT%H:%M:%f', 'now', '+9 hours')) ON CONFLICT(friend_id) DO UPDATE SET attempts = 0, available_at = excluded.available_at, last_error = NULL, updated_at = excluded.updated_at; END;

CREATE TRIGGER trg_rich_menu_rule_tag_name_update AFTER UPDATE OF name ON tags WHEN OLD.name IS NOT NEW.name BEGIN INSERT INTO rich_menu_rule_evaluation_queue (friend_id, attempts, available_at, last_error, updated_at) SELECT ft.friend_id, 0, strftime('%Y-%m-%dT%H:%M:%f', 'now', '+9 hours'), NULL, strftime('%Y-%m-%dT%H:%M:%f', 'now', '+9 hours') FROM friend_tags ft WHERE ft.tag_id = NEW.id ON CONFLICT(friend_id) DO UPDATE SET attempts = 0, available_at = excluded.available_at, last_error = NULL, updated_at = excluded.updated_at; END;
