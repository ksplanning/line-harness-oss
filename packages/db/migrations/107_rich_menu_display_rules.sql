...
CREATE TRIGGER IF NOT EXISTS trg_rich_menu_rule_tag_delete AFTER DELETE ON friend_tags BEGIN INSERT INTO rich_menu_rule_evaluation_queue (friend_id, attempts, available_at, last_error, updated_at) VALUES (OLD.friend_id, 0, strftime('%Y-%m-%dT%H:%M:%f', 'now', '+9 hours'), NULL, strftime('%Y-%m-%dT%H:%M:%f', 'now', '+9 hours')) ON CONFLICT(friend_id) DO UPDATE SET attempts = 0, available_at = excluded.available_at, last_error = NULL, updated_at = excluded.updated_at; END;
