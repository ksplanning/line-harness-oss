...
5. [ ] **[次の必須] 弾 L: メール編集 URL (埋め込み式)** — メール送信基盤新設 + 署名付き編集トークン + row_update webhook (sheets-1b の FORMALOO_ROW_UPDATE_ENABLE 設計と束ねて実装)。owner OD-A = Resend 確定済み (Phase A から着手)
