...
本案件 (A/B/C ルート分岐 jump + form_type 切替 + choice_slug モデル化) で CLOSE 済。以下は本案件外の後続:
...
- [ ] **jump 後 Prev (戻る) の再評価挙動確認** — spike で multi_step conversational renderer に上下ナビ矢印 (Prev) 存在を確認したが、back-nav 時の jump 再評価は未 clean 検証。closer live-check で確認。
