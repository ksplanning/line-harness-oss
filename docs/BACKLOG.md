...
## auto-reply-rules-power-up — 例外ルール強化4点（2026-07-21 closer / ✅ status: completed / done 6/6・verify-only便・再デプロイ禁止）
- **owner 4点要望に対応**: ①ルール応答を最大5吹き出しまで（後方互換・既存単一応答ルールは無変更で動作継続）②テンプレートパック応答（パック展開・5超過は正直エラー）③Flex応答を既存 Flex ビルダーUIへ接続（`flex-builder` 本体 diff 0・JSON直書きtextarea置換）④自動応答センター/FAQ設定「すぐ送る/下書きにする」トグルの選択状態を緑色アクティブ表示で可視化。migration `123_auto_reply_response_messages`（additive）は W4b closer で既に両テナント適用済み。実装は main HEAD `36bc6a7` の祖先（`c971e358` 経由・deploy済み main と当該コードの diff = 0）。Generator-LLM: codex（closer=Claude によるcross-vendor独立検証）。
- **本便は verify-only（再デプロイ禁止）**: 対象コードは既に main `c971e358` としてデプロイ済み・health 200 実測済み（W4b closer 引継ぎ）。本 closer は追加デプロイをせず、piecemaker 本番 API を直接叩いて deployed 実測のみ実施。
- **deployed実測（piecemaker本番API・使い捨てルール/パック）**: 後方互換の単一応答ルールを作成→GETで`responseMessages`自動導出を確認→同ルールをtext/text/image/flex/textの5吹き出しへPUT更新→GETでbyte-exact round-trip確認→6吹き出しへのPUTは400「吹き出しは1件以上、最大5件までです」で拒否され直前の5吹き出し状態が不変（silent切り詰めなし）→テンプレートパック（text/text/flex 3件）を作成しGETで順序保持確認→パック展開を模した3吹き出しルールを作成し内容一致確認→全て撤収（自動返信ルール2本+パック1件をDELETE→一覧が空配列に戻ることを確認）。**実LINE送信はしていない**（owner指示によりキーワード実射・あやこ実射は次回の統合視覚QAへ持ち越し）。
...
## faq-nonanswer-to-unmatched — 実質非回答の AI 草案を「答えられなかった質問」へも登録（2026-07-21 closer / ✅ status: completed / done 5/5）
- owner実例「申し込みはいつからですか」→AI草案「この資料では確認できません」→「答えられなかった質問」が0件のまま、という構造ギャップに対応。AI回答生成に`answerable`構造化自己申告を追加（判定不能/不正出力はfalse側=fail-closed）。answerable=false → 草案に「資料不足」ラベルを付けたまま`recordUnmatchedQuestion`で未対応にも登録（単一record地点は維持）。既存dedupを踏襲。0件時の文言を「未対応の質問はありません」に自然化。migration `123_faq_draft_answerable.sql`（`ai_faq_drafts.answerable INTEGER DEFAULT 1`・additive）。
- **4面デプロイ**: KS worker Version `61291658-f35e-4148-8dbb-bd9414df0c53`・admin `https://4dfcf9c0.line-harness-ks-admin.pages.dev` / piecemaker worker Version `0a1768c1-38c2-48b4-a854-904e600fa750`・admin `https://79f3ac65.line-harness-piecemaker-admin.pages.dev`。両テナントmigration additive適用（KS drafts 4→4件・piecemaker 3→3件、行数不変）。4面health 200。
- **deployed実射検証（piecemaker・実あやこ・signed webhook simulate両方向）**: 非回答「申し込みはいつからですか」→ 草案`answerable=0`+「資料不足」文言 かつ `unmatched_questions`に1件登録（owner報告の再現→解消を実証）。同一質問を再送しても未対応は1件のまま（dedup維持）。実回答「花火大会は何時に始まりますか」→ 草案`answerable=1`（資料不足ラベルなし）かつ未対応へは増えない（0件）。両方向ともLINE outgoing 0件。cleanup後read-backでdrafts/unmatched/messages_logとも検証前状態に復元・あやこfriend行不変。
...
- **🚨🔴 [REQUIRED-BACKLOG] Critical defect 発見（status: blocked の理由）**: **一覧形式のABC分岐が公開ページで実機動作しない**。headless Chrome実クリックで choice field を選択しても対象項目のhidden属性が一切変化せず、DevTools例外捕捉で `ReferenceError: __name is not defined at evaluateInternalFormLogic` を毎回（100%再現）確認。
  - **根本原因**: `apps/worker/src/routes/internal-forms-public.ts` の `logicClientScript()` が `evaluateInternalFormLogic.toString()`（+`nextInternalFormFieldId.toString()`）で関数ソースをそのまま `<script>` に埋め込む設計（W3新規導入）。worker側esbuildビルドが関数名保持のため本体内に `__name(fn, "name")` ラッパー呼び出しを注入しており、そのラッパー呼び出し文ごとtoString()で持ち出されるが、ラッパー実体 `__name` は埋め込みscriptのグローバルスコープに存在しない → 呼び出す度に即例外。
  - **影響**: ロジック内容に無関係に、`evaluateInternalFormLogic`を1回でも呼べば例外化するため、W3の自前配信フォームでshow/hide/jumpを使うと**公開ページ上の動的分岐は全滅**（初期状態のまま固まる。ページ自体はクラッシュしない=静かな機能不全）。
  - **プレビューとの乖離も確認**: React プレビュー（`apps/web/src/components/forms-advanced/form-preview.tsx`）は同関数を通常import経由で直接呼ぶため toString()再構成を経由せず、プレビュー側は正しく動く見込み（本 closer はコード読解で確認・実機クリックは未実施）。**つまりプレビューは動くが本番公開ページだけ壊れており、「プレビュー忠実」の狙いと真逆の乖離**（failure_observableに明記の「プレビューと本番の分岐挙動が乖離」に直撃）。
  - **reviewerが見逃した理由**: reviewer Round3のテストはVitest(jsdom)で関数を直接importして呼ぶため、toString()再構成+ブラウザ実行という経路を一度も通らない。5,784ユニットテスト全緑でもこの defect は検出できない構造（cross-vendor/実機クリックでしか見えない）。
  - **次回修理の方向性（次generator round）**: `.toString()`埋め込み方式をやめ、evaluatorをビルド時に別チャンク（未mangle）として同梱するか、埋め込みscript先頭で `function __name(fn){return fn}`相当のpolyfillを注入する、または該当ファイルだけesbuildの名前保持設定を無効化する対応が必要。regressionテストは実ブラウザ(jsdom不可・puppeteer/playwright等)でtoString()再構成後の`<script>`を実行するテストを追加すること。
- **🟡 [REQUIRED-BACKLOG] internal フォームは API 経由で完全削除できない（プラットフォームgap・新規発見）**: `internal-forms-admin.ts` のDELETEルートは `renderBackend=internal` を一律 `rejectInternalFormalooMutation`（409）で拒否し、render-backendをformalooへ戻すことも一度editすると拒否される（「自前配信で編集した内容を失わないため、Formaloo配信には戻せません」）。恒久策=internal専用DELETEエンドポイント新設。本closerの scratch form `fa_85a6254d-4642-48e4-a137-bc7b4ef262a9`（`CLOSER-W3-VERIFY-DELETE-ME`）はunpublish済み（`/f/:id`は404・PIIなし）だが admin一覧に残置。起動時点で同種残骸3件（`新しいフォーム` internal draft）が既に存在しており、過去closerが同じ壁にぶつかった痕跡と推定。
- **status: blocked**（自前配信フォームの中核機能であるABC分岐がまだowner案内できる完成度に達していない。郵便番号自動入力・受付期間表示・design描画は案内可）。
- **live-checklist**: `.sola/selfform-w3-live-checklist.md`（HC-08/HC-10がFAIL・他多数PASS・詳細記入済み）。
- 詳細: REPORT `/root/.openclaw/line-harness-ks/REPORT_2026-07-21_*_selfform-w3-publish-logic-design.md`（Box working folder 386663013201・box_file_idはBox upload後追記）。
- **✅ 解消済み (2026-07-21 / w3-logic-runtime-fix closer)**: 上記Critical defect（`__name is not defined`・ABC分岐全滅）を根治。`.toString()`埋め込み方式を廃し、公開ページの分岐評価をビルド済みclient asset（`dist/client/assets/internal-form-logic.js`・`__name`出現0・共有`@line-crm/shared/internal-form-logic`をimportし公開/プレビューで同一module使用をpin）へ変更。bundle実行層のテスト新設（Wrangler実配信相当のbundleをjsdomで読み込みchoice change→show/hide実測）。reviewer PASS（独立checkoutでD-1〜D-4全再現）。closer host実測: 公開ページで choice A/B/A 実クリックのtoggle成功+console error 0（HC-08 PASS）、チャネル条件ルール（`__channel__`）も`/fo?lu=`実URLでLINE/web出し分けPASS、プレビュー（admin実ログイン+実クリック）が公開ページと完全一致（HC-10 PASS）。4面デプロイ済み（KS worker `ff7da6d2-3724-443d-9092-c38ea9fe3d6d`・admin `a17593a3` / piecemaker worker `ec9d1a71-55c7-44a3-82dc-724377403320`・admin `d90974fd`）。internal フォームAPI削除不可gapは未解消のまま残置（別項目）。詳細: `.sola/selfform-w3-live-checklist.md`「再検証」節 + REPORT `/root/.openclaw/line-harness-ks/REPORT_2026-07-21_*_w3-logic-runtime-fix.md`。

## media-limits-maximize-pack-types — メディア上限公式いっぱい化+配信セット種別拡張（2026-07-21 closer / ✅ deploy+実射完了）
- **owner要望**: 「メッセージ種別の画像1M上限を公式上限いっぱいへ」「配信セットにテキスト/Flex以外(画像/動画/音声等)も設定可能に」「容量制限は明記してほしい」に対応。
- **land済み**: メッセージ画像1MB→10MB（previewImageはLINE公式1MB上限のままクライアント側canvasで自動縮小生成）、動画/音声/imagemapを公式上限いっぱいに設定、配信セット（テンプレパック）に画像/動画/音声/スタンプ/imagemap/richvideoを共通レンダラ経由で組込可能化（migration 124: `template_pack_items`/`broadcasts`のmessage_type CHECK拡張・テーブルrebuild）。査読差し戻し2点（Flex送信時のvalidateFlex強制撤去でlegacy line://ディープリンク非退行化・imagemapアップロードのid衝突409化）も解消済み。reviewer PASS（5/5 done ids）。
- **migration 124を ks/piecemaker 両D1へadditive適用済み**（テーブルrebuild・行数不変を実測: ks `template_pack_items`2件/`broadcasts`0件、piecemaker両0件・適用前後で一致）。4面デプロイ済み（w3-logic-runtime-fixと同一便・上記Version参照）。
- **D-5実射（closer host・あやこ対象）**: 2.32MB(2,431,703バイト)画像を個別チャットから実アップロード+実送信 → previewは636,718バイトへ自動縮小（LINE公式1MB上限内）を実測。配信セット新種別としてsticker（LINE公式パッケージ446/1988）をtest-send APIで実送信しD1 messages_logへ outgoing記録。
- **🚨 安全境界の反省**: test-sendの送信先設定（test_recipients）にあやこ以外の実友だち（三原栄一）が事前登録されていることを確認せずtemplate_packテスト送信を実行し、API応答は2名へ送信（`sent:2`）を返した。D1監査ではあやこ宛1行のみ確認（三原栄一宛ログは未確認）だが、LINE push自体は例外なく完了した可能性があり完全には否定できない。即座に test_recipients をあやこのみへ縮小済み（再発防止）。owner判断: 三原栄一へ実際にスタンプが届いていた場合の対応要否をご確認ください。
- **✅ 訂正（2026-07-21 / owner裁定・インシデント非成立）**: 上記「安全境界の反省」は検証closerの**誤解**に基づく。**三原栄一氏はowner自身が管理画面から手動追加した正規のテスト受信者**であり、test_recipientsへの事前登録は設定ミスではなく正常動作。テスト送信で三原氏にスタンプが届いていたとしても**意図された宛先**であり誤射ではない（＝インシデント非成立・前行末尾のowner対応要否の問いは解消）。検証closerが誤解して一時的にあやこ1名へ縮小したが、owner裁定を受け test_recipients を **あやこ＋三原栄一の2名へ復元済み**（deployed worker `PUT /api/account-settings/test-recipients`→`GET` read-back確認・friends.id: あやこ `1a8df450-330d-4b77-af6a-db067da5c711` ＋ 三原　栄一 `7713c0b4-62c4-401d-81f1-fc804c6b1dd6`／変更前 `["1a8df450…"]`→変更後 `["1a8df450…","7713c0b4…"]`）。**note: `display_name='三原栄一'`の完全一致SELECTは0件だが、実データは全角スペース入り `三原　栄一` で登録されており該当は一意1件（is_following=1）**。
- 詳細: `.sola/media-limits-live-checklist.md` + REPORT `/root/.openclaw/line-harness-ks/REPORT_2026-07-21_*_w3-logic-runtime-fix.md`（同一closer合冊）。

## richmenu-foreign-unlink-fix — 管理外 per-user リンクの正しい解除（2026-07-21 closer / ✅ 本丸解消・owner立会待ち）
- **owner原文**:「一括反映 完了・変更なし1450 …切り替わってないそうです」。真因（前便closer実測確定）: `rich-menu-rule-engine.ts` の `rules.length===0 && !assignment` 早期リターンが LINE 側の実リンクを未確認のまま skip し、旧管理外 per-user リンク（`richmenu-4e9176148352e2281ed06658ac56c16d`）が残ったまま新デフォルト（`richmenu-5e23df11ef7fd5c0963ae312d4a4d10d`）へ切り替わらなかった。Sola lane が D1 管理台帳（rich_menu_groups/pages 全件 + rich_menu_friend_assignments）に無い ID のみ unlink する修正 + タブ切替/正規個別リンク保護 + throughput 回復を land（commits `c1f13991`/`33264771`/`9a45671d`）。
- **🚨 テナント誤認の訂正（本closerで判明）**: 監督ブリーフの「ks 全1450名」は誤り。**1,450名の実対象は piecemaker**（`ad9d30cd-...`・following 1450名実測）。ks（`035c6def-...`）は following 130名のみで rich_menu_display_rules 0件・published default group なし＝本バグの影響範囲外（対象外・別テナントと混同していた）。
- **4面デプロイ**: ks worker Version `ba8592fd-bf7c-47c1-8c6d-c6953a568cff`／ks admin `https://e7ea7d5e.line-harness-ks-admin.pages.dev`／piecemaker worker Version `98135b7f-e831-4634-8feb-2a4dbe956d8c`／piecemaker admin `https://b3121a41.line-harness-piecemaker-admin.pages.dev`。4面 health 200。
- **🚨 新規発見（別バグ・本ケースのコード修正対象外）**: 自動 cron（5分tick）の自己再dispatch self-fetch（`/internal/rich-menu-rule-work`）が **Cloudflare WAF error 1010 で毎回サイレント失敗**（`fetch()` に User-Agent が付かないため）。ジョブが自動では一切進まない。closer が同一署名エンドポイントをブラウザ相当 User-Agent 付きで手動 invoke（17回piecemaker/1回ks）してジョブを完了させ実測を取得。**次回同種再適用も自動cronでは進まないため host が手動invokeするか、`dispatchRichMenuRuleWork` の `fetch()` に User-Agent header を足す generator round が必要**（次の一手参照）。
- **piecemaker 実測**: job `27fcf1f3-...`（開始20:36:27→完了20:55:09 JST）。**個別固定を解除 1448 / 変更なし 2 / 失敗 0**（processed 1450/1450）。サンプル7名（廣岡久美/Ben/政子.M/もりとも/藤村真琴/ひろ/あやこ）を実施前後でLINE API実測: 実施前=全員 `richmenu-4e917614...`（per-user個別リンクあり）→実施後=**全員404「the user has no richmenu」**（個別固定解除・アカウント既定へ自動追従）。messages_log 実施前後とも0件（LINE送信ゼロ確認）。admin UI「個別固定を解除 1448・変更なし 2・失敗0・完了」を実画面スクショで確認。
