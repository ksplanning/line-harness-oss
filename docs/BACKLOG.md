...
**reviewer PASS Round 1（Critical/High 0・closer_allowed true）。closer 統合: main commit `73766f0`（merge commit・origin+piecemaker 両 push・SHA一致確認済み）。ks/piecemaker 両 worker+両 admin 再デプロイ済み（health 4点 200・⑤回帰 live-check: fa_5127eb98…b0481 の /fo が peace-maker.formaloo.me/puw7lh へ 302 継続確認）。closer 独立検証(Codex cross-vendor)で①③のみ即PASS・②④とR-4はCodexが過度に文字通り解釈した誤検出と closer が判断(コード実読+既存test精査で実質PASS)・⑤に実質的ギャップ1件発見(下記)。status: blocked**（⑤のギャップ修理待ち。owner 実機確認は別途、ギャップ修理後に実施）。

### ⚠️ closer 独立検証で発見したギャップ（要 generator follow-up）
- **⑤(a) GET フォールバック条件が狭すぎる**: `formaloo-sync.ts:90` の GET 再取得は `if (!publicAddress)`（`full_form_address ?? address ?? null` の結果が両方欠落の時のみ）で発火する。しかし実際の本番事故シナリオ（`docs/shared-runbooks/app-profiles/line-harness-piecemaker.md` 記載）は「create 応答に `address`(裸コード) はあるが `full_form_address` が無い」ケースであり、この場合 `publicAddress` は裸 `address` で truthy になり GET 再取得が**発火しない**（test も「slug のみ・address 無し」しかカバーしておらず、この核心シナリオ未検証）。**実害は buildPublicUrl の絶対https gate が裸コードを null 化するため「壊れたリダイレクト」にはならない(safe-fail)が、⑤ が本来解決すべき「次の新規フォームでも自動的に正しい公開URLが手に入る」が未達のまま**。同一 account (peace-maker / address_is_universal:false) で次に新規フォームを作ると同じ手動DB修理が再度必要になる見込み。

### 🎯 次の必須（required backlog）
- [ ] **[REQUIRED-BACKLOG] ⑤(a) GET フォールバック条件修正**: `formaloo-sync.ts` の GET 再取得トリガーを「`full_form_address` 欠落」の直接判定に変更（`address` の有無に関係なく）。回帰test追加（「address はあるが full_form_address が無い」ケース）。修理後に owner 実機スモークへ。
- [ ] owner 実機スモーク（⑤修理後）: ①未同期フォームで「今すぐ同期」ボタンが目立って表示され、押すと再送されるか ②プレビューで文字を入力でき、残り文字数カウンターが動くか（送信は相変わらずできない） ③保存・公開後にビルダー上から公開ページを開いてテストできるか。OK なら completed に昇格。
