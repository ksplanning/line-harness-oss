...
5. [ ] **弾 L: メール編集 URL (埋め込み式) — Phase A** — 署名付き編集トークン + 公開編集ページ (`/fe/:token`) + builder トグル。
   ...
   (T-A1〜T-C2)。**ただし closer live smoke (使い捨てフォーム+row+実 Formaloo API) で保存機能の重大バグを発見
   (`apps/worker/src/routes/formaloo-public.ts:588` persist確認が実API shape `data.data.row.data` でなく
   `data.data` を読んでいるため常に反映確認失敗・route test の mock shape 乖離が原因・修正候補明記済み)。
   GET描画/改ざん拒否/トグルOFF拒否は実測PASSだが save が動かないため `status: blocked`**。
   詳細 REPORT_2026-07-17_135634_form-edit-mail-link.md 参照 (box_file_id_md `2352219628114`)。
5.0.1 [ ] **[REQUIRED-BACKLOG] 弾L save バグ hot-fix + 再検証** — `formaloo-public.ts:588` を
   `verifyRes.data?.data?.row?.data` に修正 + `formaloo-public-edit.test.ts` の stub fetch shape を実API相当に
   修正 → reviewer 再検証 → closer が同じ live smoke 手順で (b) save→FRESH GET 実反映 が PASS することを再確認
   してから弾L Phase A を completed へ昇格。**Phase B (メール送付) 着手より優先**。
5.0.2 [ ] **[次の必須] 弾 L Phase B: メール送付** — 送信完了時に編集用URLをメールで自動送付 (T-D1/T-D2/T-D3/S-4)。
   ...
   **5.0.1 の save バグ修正が前提条件**(壊れた保存機能の上にメール送付を積まない)。
...
> **相互参照**: デザイン未設定フォーム（テーマ色/カバー未設定のまま公開）での配色事故（既定色が意図せず視認性を損なう等）は、本案件のスコープ外。恒久対策は `form-design-presets` 案件（planner 走行中）で扱う。B1 のパレット追加自体は既存デザイン機構（form-design-theme・completed 済）に影響しない additive 変更。
