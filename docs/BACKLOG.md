...
**⚠️ 発見事項（新規 defect / 要 follow-up・generator 未着手）**: `confirmFormCopyReflected`（GET-after-PATCH の soft-200 対策確認）が **success_message のみ**常に不一致判定を返し、route が `out_of_sync`（`文言が公開ページに反映されませんでした（success_message）`）を誤って surface する。**ks/piecemaker 両テナントで再現**（button_text/error_message は確認 OK・success_message のみ）。しかし上記の通り **hosted 実描画は正しく日本語で反映される**（false negative = 「保存済なのに未反映と誤警告する」パターンで、逆の「未反映なのに保存済と誤認させる」殻完了パターンではない）。想定原因: Formaloo GET `/v3.0/forms/{slug}/` 応答の envelope で `success_message` の実 key/位置が `button_text`/`error_message` と異なる（confirmDesignReflected 由来の `extractForm` 前提と食い違う可能性・id-191 と同型の fixture-vs-reality 疑い）。closer は独立検証のみ実施（コード修正はしない）。

### 🎯 次の必須（required backlog）
- [ ] **success_message の false out_of_sync 根治**: `confirmFormCopyReflected`（`apps/worker/src/services/formaloo-copy.ts`）が実 Formaloo GET 応答で success_message をどのキー/形で返すか実測し、判定ロジックを是正する。現状は製品挙動（hosted 反映）に実害はないが、owner/staff が管理画面で「同期失敗」の誤警告を見続ける（信頼性低下）。
