...
### 🎯 次の必須（required backlog）
- [ ] **[REQUIRED-BACKLOG] owner 実機スモーク（色の見た目確認・本案件の核心確認）** — フォームビルダーで dark-sumi 等ダークプリセットを選択→保存→公開ページを owner が実機（スマホ/PC）で開き、①背景・ボタンが選んだプリセット通りの色に見えるか ②入力欄の文字がちゃんと読めるか を確認。②は closer 実測で安全側（contrast>=4.5）と確認済みだが①は closer の headless テストで未反映だったため要実機確認。NG なら Formaloo 側のテーマバインディング（`system_theme` オブジェクト）周りの追加実装が必要になる可能性。
- [ ] **[REQUIRED-BACKLOG] Formaloo hosted の色反映タイミング/キャッシュ調査** — closer の headless-chrome 実測で dark-sumi と line-green が公開直後（数分以内）に視覚的に同一レンダリングだった原因を調査（CDN cache TTL / system_theme バインディング要否 / 反映に要する時間）。form-design-theme 案件は owner 実機で「OK」だったため、既存機能が壊れているのではなく本 closer のテスト手法・タイミングに起因する可能性が高い。
