...
**reviewer PASS Round 2**（Round1 FAIL F1-F4 High → 3 commit 修正 → 全解消・新規Critical 0）。**status: pending_owner_confirmation** — owner スマホスモーク待ち（form-builder-ux Batch A/C と同型）。

### 🎯 次の必須（required backlog）
- [ ] [REQUIRED-BACKLOG] owner スマホ実機スモーク: フォームビルダー →「デザイン」タブ → プリセット選択 or 個別色 → ロゴ/カバー画像設定 → 保存 → 公開ページ URL で色・画像の反映確認 → OK なら completed に昇格。
