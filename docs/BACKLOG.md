...
**⚠️ 発見事項（新規 defect / 要 follow-up・generator 未着手）**: **背景「全面」（`background_image` / カバー画像 multipart PATCH）は Formaloo API 上は正しく保存される（GET で S3 URL 確認・applyDesignImages 契約どおり）が、公開ページ上で視覚的に描画されない**（ks/piecemaker 両テナント・計 2 回ずつ・headless Chrome + 実時間 CDP 10 秒待機の双方で再現）。同一フォームで色（テーマ）は正しく描画されるため、これは push パイプライン全体の不調ではなく **背景画像レンダリングに固有の欠落**。仮説（未検証・要 further investigation）: Formaloo のフロントは `background_image`（フラット・トップレベル文字列フィールド）ではなく、GET レスポンスに常に空 `{}` で観測される **`theme_config.background_image`（入れ子オブジェクト）** を実際の描画判定に使っており、`applyDesignImages()` はトップレベルの `background_image` のみ PATCH してこの入れ子を更新していない可能性がある。2026-07-18 spike (S-2) は本ケース対象と異なる Formaloo アカウント（`sukedachi-hp.formaloo.me`）で成功したため、アカウント/フォーム種別差の可能性も排除できていない。

### 🎯 次の必須（required backlog）
- [ ] **背景「全面」の hosted 描画欠落を追究・修理** — `theme_config.background_image` 入れ子オブジェクトの役割を実機調査（Formaloo サポート確認 or 別 live probe）。恒久解が判明するまで、admin UI の「全面」オプションは「反映されない可能性がある」ことを owner に案内するか、確実性が取れるまで選択肢を一時的に隠す判断を検討（owner 判断）。
- [ ] **O-3 owner 立会スモーク（ks/piecemaker 両）** — 差し込み画像（幅調整）と帯モードは owner が実機で試して OK を出せる状態。**全面モードは上記欠落があるため owner スモークで確認しても意図通りに見えない可能性が高い**（先に伝えること）。
...
### 🎯 次の必須（required backlog）
- [ ] **T-C3: 再取り込み時に formRedirect/formRedirectTouched をリセットする**（builder.tsx handleReimport に successPages と同型の処理を追加）。
- [ ] **T-E5: SP 本文変更検知の恒久解**（drift cron のゲートを fingerprint 単独でなく SP 本文差分も考慮する形に見直すか、SP 本文変更は drift とは別の同期経路で扱う方針を再設計）。
