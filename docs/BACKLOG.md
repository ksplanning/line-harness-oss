...
2. [ ] **form-design-theme (第4弾)** — テーマ色/カバー/ロゴ。planner 走行中 (.plans/2026-07-16-form-design-theme/)
...
- [OPTIONAL-POLISH] `apps/web/tsconfig.tsbuildinfo` が working-tree dirty のまま（reviewer 指摘・cosmetic のみ・commit 混入なし）。次回 promote 前に revert すると worktree clean（gating 非対象・任意）。
