...
- [x] [REQUIRED-BACKLOG] Batch C（プレビュー）着工 — Batch B 完了後（装飾型に依存）。2026-07-16 closer 統合・両テナントデプロイ完了。**owner スマホ実機スモーク確認待ち（下記6項目）で completed 昇格**。
- [ ] owner スマホ実機スモーク（Batch C）: ①フォームビルダーを開く ②部品組立（DnD/タップ追加） ③装飾追加（見出し+説明/改ページ） ④タイトル編集 ⑤プレビュー確認（スマホはタブ切替・見出し/説明文/必須マークが実物イメージで見える）。「OKでした」等の確認で completed 昇格。
