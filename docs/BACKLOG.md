...
- [x] 両テナント適用（dual-push: ks + piecemaker 両 admin 再デプロイ）— Batch A + Batch B land 分完了。Batch C は land 時に同様実施要。

### 🎯 次の必須（required backlog）
- [x] owner スマホ実機スモーク（R-3）: 375px タッチで「部品を長押し→ドラッグで追加／並べ替え／掴んだ部品が指に追従／置き場所が光る+挿入位置表示／枠外リリースでメッセージ／タップ追加が従来どおり／縦スクロールが死んでいない」7項目を owner が実機確認 → 2026-07-16「OKでした」で Batch A completed に昇格。
