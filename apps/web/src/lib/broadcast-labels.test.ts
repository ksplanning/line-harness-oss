...
describe('messageTypeLabels — 全 7 種別の日本語ラベル', () => {
  it('各種別が正しい日本語ラベルに mapping される', () => {
    ...
    expect(messageTypeLabels.sticker).toBe('スタンプ')
...
describe('mediaPreviewSummary — 新 type は生 JSON を出さず簡潔サマリ', () => {
  ...
  it('sticker → IDsを読める形で表示し、生JSONを出さない', () => {
    const s = mediaPreviewSummary('sticker', JSON.stringify({ packageId: '446', stickerId: '1988' }))
    expect(s).toContain('446')
    expect(s).toContain('1988')
    expect(s).not.toContain('packageId')
  })
