...
describe('sheetsConnectionsApi', () => {
  ...
  test('create/update/delete send only the connection settings contract', async () => {
    ...
    await sheetsConnectionsApi.update('gsc/1', updateInput)
    expect(fetchApiMock).toHaveBeenNthCalledWith(2, '/api/integrations/google-sheets/connections/gsc%2F1', {
      method: 'PATCH', body: JSON.stringify(updateInput),
    })

    await sheetsConnectionsApi.remove('gsc/1')
    expect(fetchApiMock).toHaveBeenNthCalledWith(3, '/api/integrations/google-sheets/connections/gsc%2F1', {
      method: 'DELETE',
    })
  })
  ...
  test('connection test returns the boolean result', async () => {
    ...
    await expect(sheetsConnectionsApi.test('gsc/1')).resolves.toBe(false)
    expect(fetchApiMock).toHaveBeenCalledWith('/api/integrations/google-sheets/connections/gsc%2F1/test', {
      method: 'POST',
    })
  })
})
