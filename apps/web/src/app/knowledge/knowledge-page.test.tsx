...
describe('/knowledge page — AI ログ・コストタブ', () => {
  ...
  it('answerable=false の草案だけに資料不足ラベルを表示する', async () => {
    ...
    const sourceLimitedCard = screen.getByText('Q: 申し込みはいつから？').closest('.border')
    const answeredCard = screen.getByText('Q: 営業時間は？').closest('.border')
    expect(sourceLimitedCard).not.toBeNull()
    expect(answeredCard).not.toBeNull()
    expect(within(sourceLimitedCard as HTMLElement).getByText('資料不足')).toBeTruthy()
    expect(within(answeredCard as HTMLElement).queryByText('資料不足')).toBeNull()
  })
})
