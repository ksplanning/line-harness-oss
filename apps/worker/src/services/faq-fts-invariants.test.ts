...
describe('D-2 — dark-ship gate + 検索スコア下限 + escalate/grounding/LLM 層 保持', () => {
  test('wrangler.ks.toml の crons=[] / FAQ_BOT_ENABLED="false" が byte-identical (dark-ship / compat flag は additive)', () => {
    ...
    // (crons=[] / FAQ_BOT_ENABLED の 2 行は不可侵)。
    const cur = toml.split('\n');
    const main = execFileSync('git', ['show', 'origin/main:apps/worker/wrangler.ks.toml'], { cwd: REPO }).toString().split('\n');
    expect(cur.length).toBe(main.length); // 行追加/削除なし (in-place 変更のみ)
    const changed = cur.filter((l, i) => l !== main[i]);
    for (const l of changed) expect(l).toMatch(/compatibility_flags/);
  });
