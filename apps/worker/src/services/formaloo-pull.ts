...
export function buildPullResult(
  ...
  //   target 空 (既定完了ページ) を drop しない (Phase2 の success-page target は別集合照合)。
  const b5Keep = (r: HarnessLogicRule): boolean => {
    if (!idSet.has(r.sourceFieldId)) return false;
    if (r.action === 'submit') return r.targetFieldId === '' || idSet.has(r.targetFieldId);
  ...
  const logic = Array.isArray(rawLogic)
    ...
    : fromFormalooLogic(logicObj, resolveId).filter(b5Keep);
