...
編集URLまたは回答シートから変更された回答だけを「未承認」として見つけ、内容を確認して承認すると一覧から消えることを確認する。lane 内では migration/DAO/API/UI の自動テストまで実施済みで、実DB migration と trusted host のブラウザ確認は host closer が行う。
...
1. [ ] 査読済みの migration `133_internal_form_submission_external_edit_review.sql` を第1テナントへ適用し、成功を確認する。
2. [ ] 同じ migration を第2テナントへ適用し、成功を確認する。
3. [ ] 両テナントで `internal_form_submissions` に `external_edit_source`、`external_edited_at`、`external_edit_approved_at` が追加され、既存回答の3列がすべて `NULL` であることを確認する。
...
3. [ ] 「外部編集（未承認） N件」を押し、編集した回答だけが表示され、経路が「編集URL」になっていることを確認する。
4. [ ] 回答本文を確認して「確認して承認」を押し、その行が一覧からすぐ消え、件数が1件減ることを確認する。
5. [ ] 既定表示へ戻すと回答自体は残り、回答本文・送信日時・添付が承認前と同じであることを確認する。回答シートとLINEには書き込み・送信が発生していないことも確認する。
6. [ ] 同じ回答を編集URLからもう一度変更し、再び未承認一覧へ現れ、経路が「編集URL」、承認状態が未承認へ戻っていることを確認する。

## 回答シート → 絞り込み → 承認

1. [ ] 第2テナントの回答シートで架空回答の回答セルを変更し、通常の polling/manual 同期で取り込む。
2. [ ] 回答データの「外部編集（未承認）」一覧に同じ回答が現れ、経路が「シート」になっていることを確認する。
3. [ ] 内容を確認して承認し、一覧からすぐ消えること、回答本文とシートのセルへ承認由来の追加書き込みがないことを確認する。
4. [ ] 別の架空回答をシートで変更し、webhook 同期でも未承認一覧へ現れて経路が「シート」になることを確認する。
5. [ ] 管理画面の通常の回答編集だけを行った回答は、未承認件数が増えず、外部編集一覧へ入らないことを確認する。

## 画面幅・記録欄

- [ ] desktop と 375 × 812 の両方で、件数付きボタン、経路表示、承認ボタンが読め、承認後に空表示または次の行へ自然に切り替わることを確認する。
...
- 編集URL経路の出現 / 承認 / 再編集結果:
- polling/manual と webhook のシート経路結果:
- 管理画面編集が対象外であること:
