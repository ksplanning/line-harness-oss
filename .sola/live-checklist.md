# ai-reply-style — host closer 実測チェック（2026-07-24）

## 目的と安全境界

LINEアカウント別に保存した返信スタイルと名乗りが、AI下書きの実生成へ反映され、別アカウントや未設定時へ漏れないことをtrusted hostで確認する。lane内では生成スタブだけを使い、外部LINE送信は0件。以下は査読済みrevisionを確認環境へ反映した後にhost closerが実施する。

- [ ] 査読済みrevisionをtrusted hostへ反映し、additive migration `144_ai_reply_style_defaults.sql` が対象D1へ適用済みであることを確認する。lane branchを本番へ直接deployしない。
- [ ] ownerが使用を許可した確認用LINEアカウントA/Bと、個人情報を含まない架空の確認質問だけを使う。実在顧客、未処理の問い合わせ、運用中の会話で代用しない。
- [ ] A/Bとも回答モードを「下書き」にしてから実測する。「自動で送信する」へ切り替えず、LINE返信API、個別送信、一斉配信、シナリオ、テスト送信を呼ばない。
- [ ] token、cookie、LINE user ID、実名、実際の会話本文をNetwork記録・スクリーンショット・結果欄へ残さない。安全な確認資材や下書き生成導線がなければ`BLOCKED`とする。
- [ ] スタイル欄には言葉遣い・口調・文章の形だけを書く。ナレッジの事実変更、秘密情報、顧客情報、外部送信を求める文は入力しない。

## 保存往復とA/B分離を確認する

1. [ ] 自動応答センターでAを選び、「返信スタイル」に次の確認用設定を入力する。
   - 返信スタイルの指示: `です・ます調で、親しみやすく、2文以内で簡潔に答える。`
   - 名乗り（冒頭文）: `確認用の◯◎です。`
2. [ ] 注意書きに「資料やFAQの事実を変える指示を入れない」「登録済みナレッジを優先する」という意味が表示されることを確認し、「設定を保存」を1回だけ押す。
3. [ ] 再読込してAを選び直し、2項目が保存前と1文字単位で一致することを確認する。Networkの設定GET/PUTがAの`accountId`だけを持ち、秘密値を含まないことを確認する。
4. [ ] Bへ切り替え、Aの2項目が表示されず空欄であることを確認する。Bへ別の確認用スタイルを保存し、Aへ戻ってもAの値が変わらないことを確認する。

## 実生成の変化とownerの好みを確認する

1. [ ] A/Bとも下書きモードであることを再確認する。まずAのスタイルを一時的に空欄で保存し、登録済みの確認用ナレッジだけで答えられる同じ架空質問を1回入力して、基準となるAI下書きを作る。LINE送信・着信が0件であることを確認する。
2. [ ] Aへ手順1のスタイルと名乗りを再保存し、同じ確認質問でAI下書きをもう1件作る。下書きの先頭が正確に`確認用の◯◎です。`で始まり、その直後の本文が指定した親しみやすいです・ます調になっていることを確認する。
3. [ ] 基準下書きとスタイル設定後の下書きが、同じ登録済みナレッジの事実・URL・電話番号・数値を使っていることを確認する。口調と文章の形以外の事実が追加・変更されていたら`FAIL`とする。
4. [ ] ownerがAのスタイル設定後下書きを読み、「この名乗り・口調を日常運用で使いたいか」を確認する。修正希望があればスタイル欄だけを調整し、最大2回まで再生成して採用文を確定する。ナレッジ本文を好みに合わせて書き換えない。
5. [ ] Bのスタイルを空欄へ戻し、同じ確認質問で下書きを1件作る。Aの名乗り・スタイル文が0件で、Bのナレッジだけから生成されることを確認する。

## 未設定回帰と撤収を確認する

1. [ ] A/Bの返信スタイルと名乗りを両方空欄で保存し、再読込後も空欄であることを確認する。空の専用スタイル文や余分な冒頭改行が下書きへ表示されないことを確認する。
2. [ ] laneの機械証跡で、未設定時のauto/draft promptと生成文が旧値と完全一致し、auto/draftが同じprompt組立関数を通ることを照合する。hostではLLMの非決定性を理由に文章の完全一致を要求しない。
3. [ ] 確認用下書き、架空質問、Bへ一時保存したスタイルをownerの手順で撤収する。設定を継続利用する場合だけ、ownerが採用したAのスタイルを残す。
4. [ ] Networkと確認用端末を照合し、LINE返信API 0回、個別送信0回、一斉配信0回、シナリオ送信0回、LINE着信0件、実顧客更新0件であることを確認する。

## 実測結果

- 実施日時（JST）:
- 査読済みrevision / deploy revision / trusted host:
- migration 144適用確認:
- 確認用LINEアカウントA / B（秘密値・個人情報を書かない）:
- Aの保存前後一致 / Bへの非漏洩 / BからAへの非漏洩:
- 未設定時の基準下書き:
- スタイル設定後の名乗り先頭一致 / 口調・文数:
- 基準と設定後のナレッジ事実差分:
- owner好み確認 / 採用したスタイル（秘密値・顧客情報を書かない）:
- B生成へのAスタイル混入数（0）:
- LINE返信API数（0）/ その他送信数（0）/ LINE着信数（0）:
- 実顧客更新数（0）/ 撤収結果:
- 結果（PASS / FAIL / BLOCKED）と理由:

---

# draft-to-faq — host closer 実測チェック（2026-07-24）

## 目的と安全境界

AI下書きの「よくある質問に追加」が既定OFFで、選んだ時だけ送信成功後に同じLINEアカウントのFAQへ編集可能な無効状態で入ることをtrusted hostで確認する。lane内では外部LINE送信を行わず、以下は査読済みrevisionを確認環境へ反映した後にhost closerが実施する。

- [ ] ownerが使用を許可した確認用LINEアカウントA/Bと、架空名・個人情報を含まない本文だけの確認用友だちを使う。実在顧客、未処理の問い合わせ、本番FAQで代用しない。
- [ ] A/Bそれぞれの確認用端末以外へ送信しない。承認送信は下記OFF 1件・ON 1件の合計2件だけに限定し、通常返信、一斉配信、シナリオ、テスト送信は行わない。
- [ ] token、cookie、LINE user ID、実名、実在する本文をNetwork記録・スクリーンショット・結果欄へ残さない。安全な確認資材がなければ`BLOCKED`とする。
- [ ] 本件にmigrationはない。schema変更や本番D1の手動INSERT/UPDATE/DELETEで状態を作らない。
- [ ] FAQ登録失敗の強制試験はlaneの機械テストが正本。trusted hostに安全な既存fault-injection手段がない場合、table破壊や権限変更で再現せず、host実測対象外と記録する。

## 既定OFF・注意書き・状態リセット

1. [ ] Aを選び、個別チャットでpendingのAI下書きを開く。「よくある質問に追加」チェックと、「今後の自動応答の参考」「FAQ一覧で編集」「個人情報を確認」の意味が読める注意書きが表示されることを確認する。
2. [ ] チェックがOFFであることを確認し、チャット履歴の拡大表示を開く。通常表示でON/OFFを切り替えると拡大表示にも同じ状態が反映され、逆方向の切替も同期することを確認する。
3. [ ] 一度ONにして回答を編集・保存し、同じ下書きでは選択が維持されることを確認する。その後OFFへ戻す。
4. [ ] 自動応答センターの下書き受信箱でも同じチェック・注意書きが表示され、既定OFFであることを確認する。

## OFF送信・ON送信・FAQ一覧往復

1. [ ] Aの確認用下書き1件をチェックOFFのまま「承認して送信」する。Networkのapprove requestが`addToFaq:false`で200を返し、Aの確認用端末に回答がちょうど1通届くことを確認する。
2. [ ] AのFAQ一覧を再取得し、手順1の質問・回答を持つ新規FAQが0件であることを確認する。次に表示した別のAI下書きのチェックがOFFへ戻っていることも確認する。
3. [ ] Aの別の確認用下書きでチェックをONにし、「承認して送信」を1回だけ押す。approve requestが`addToFaq:true`で200を返し、Aの確認用端末に回答がちょうど1通届くことを確認する。
4. [ ] 送信後にAのFAQ一覧を再取得し、顧客メッセージの質問と実際に送った回答に一致するFAQがちょうど1件増えたことを確認する。架空の友だち表示名やprofile情報が質問・回答へ自動付加されていないことを確認する。
5. [ ] 追加FAQが「無効」で、自動応答にはまだ使われない状態であることを確認する。「編集」を開き、質問・回答・有効状態を変更できることを確認して、保存せず閉じる。
6. [ ] ON送信後に次のAI下書きを表示し、チェックがOFFへ戻っていることを確認する。

## A/B分離・既存フロー回帰・撤収

1. [ ] Bへ切り替えてFAQ一覧を再取得し、Aで追加したFAQが0件であることを確認する。下書き受信箱にもAのpending下書きが表示されないことを確認する。
2. [ ] Aへ戻ると追加FAQが1件だけ存在し、質問・回答・無効状態が手順4〜5と一致することを確認する。
3. [ ] 送信に使わない別の確認用下書きで、本文編集の保存と破棄確認が従来どおり動くことを確認する。破棄は確認用下書き1件だけに限定する。
4. [ ] Networkと確認用端末を照合し、approve POSTが2回、LINE着信がAへ2件、Bへ0件、通常送信・一斉配信・シナリオ送信が0件であることを確認する。
5. [ ] 追加した確認用FAQ、確認用下書き、架空友だちをownerの手順で撤収する。FAQを有効化して自動応答へ残さない。
# autoreply-actions — host closer 実測チェック（2026-07-24）

## 目的と安全境界

例外ルールの返信がLINEへ正常送信された後だけ、同じ友だちへタグとカスタムフィールドのアクションが設定順に反映されることをtrusted hostで確認する。lane内では実LINE送信や実顧客データの変更を行わず、以下は査読済みrevisionを確認環境へ反映した後にhost closerが実施する。

- [ ] 査読済みrevisionをtrusted hostの確認環境へ反映し、additive migration `143_auto_reply_actions.sql` が対象D1へ適用済みであることを確認する。lane branchを本番へ直接deployしない。
- [ ] ownerが使用を許可した確認用LINE公式アカウントと架空の友だち1人だけを使う。実在顧客、未処理の問い合わせ、運用中ルールで代用しない。
- [ ] 確認専用のキーワード、タグ2個、カスタムフィールド2個を新規に用意する。既存タグ・既存フィールドの値を変更しない。
- [ ] token、cookie、完全なLINE user ID、個人名、実際の会話本文をNetwork、ログ、スクリーンショット、結果欄へ残さない。安全な確認資材を用意できなければ`BLOCKED`とする。
- [ ] 実測前に確認用友だちへ「外すタグ」を付け、「設定する項目」と「空欄にする項目」へ撤収可能な架空値を入れる。変更前の確認用状態だけを控える。

## migrationと保存往復を確認する

1. [ ] 読取専用の`PRAGMA table_info(auto_replies)`で`on_reply_actions_json`が追加され、既存の確認用ルールでは`NULL`であることを確認する。migration前からあるルールの件数・キーワード・返信内容が変わっていないことも確認する。
2. [ ] 自動応答センターの例外ルールで、確認専用キーワードに対して次の4件をこの順で設定する。
   1. 「タグを付ける」→確認用の追加タグ
   2. 「タグを外す」→確認用の削除タグ
   3. 「カスタム項目に値を入れる」→確認用状態=`自動応答済み`
   4. 「カスタム項目を空欄にする」→確認用メモ
3. [ ] 保存してダイアログを閉じ、一覧を再読込して同じルールを開き直す。4件の種類・対象・値・順序が1文字単位で一致することを確認する。
4. [ ] DevTools Networkの`GET /api/auto-replies`または`GET /api/auto-replies/:id`で、`replyActions`が画面と同じordered arrayで返ることを確認する。認証headerや他ルールの本文は保存しない。

## 送信成功後・再実行・fail-safeを確認する

1. [ ] 確認用友だちから確認専用キーワードを1回送る。LINE返信がちょうど1回届いた後に、追加タグあり・削除タグなし・確認用状態=`自動応答済み`・確認用メモが空欄、の4点になることを確認する。
2. [ ] 同じ友だちから同じキーワードをもう1回送り、返信は今回分の1回だけ、タグ重複0件、削除タグなし、2項目の最終値が手順1と同じであることを確認する。
3. [ ] trusted hostに安全なpostback確認導線がある場合だけ、確認専用postback値へ同じ4アクションを設定し、postback返信成功後にも同じ最終状態になることを確認する。安全な導線がなければこの小項目だけ`BLOCKED（確認用postback導線なし）`と記録し、運用中リッチメニューを変更しない。
4. [ ] fail-safe確認用ルールへ、保存後に削除した確認用タグを対象にする失敗アクション、その後に有効な確認用フィールド設定を並べる。キーワードを1回送り、返信が正常に届き、先頭アクションの失敗後も後続フィールドが反映されることを確認する。
5. [ ] LINE返信を安全に失敗させられるtrusted hostのfixtureがある場合だけ、確認用reply tokenの失敗ケースを実行し、アクションが0件であることを確認する。fixtureがなければこの小項目だけ`BLOCKED（安全な送信失敗fixtureなし）`とし、実アカウントのtokenや通信を故意に壊さない。
6. [ ] friend未連携を安全に再現できるhost test fixtureがある場合だけ同じ設定を実行し、全アクションが`friend_not_linked`でskipされ、タグ・フィールドが変わらないことを確認する。fixtureがなければこの小項目だけ`BLOCKED（安全な未連携fixtureなし）`とし、実友だちの紐付けを削除しない。

## 未設定回帰と撤収を確認する

1. [ ] migration前からあるアクション未設定の例外ルールを1件開き、「応答後にやること」が未設定であることを確認する。設定を変更せず、従来どおりの返信内容・吹き出し数・未対応リスト状態になることを確認する。
2. [ ] アクション未設定ルールを1回発火し、返信が従来どおり1回届き、確認用友だちのタグ・カスタムフィールドに変更が0件であることを確認する。
3. [ ] silentルールへ確認用アクションを一時設定し、キーワード一致時に返信もアクションも発生しないことを確認する。確認後は設定を空配列へ戻す。
4. [ ] 確認用ルール、タグ、フィールド、架空値、必要なら確認用友だちをownerの撤収手順で削除する。Networkで想定外のLINE送信0件、実顧客の更新0件であることを確認する。
...
- 査読済みrevision / deploy revision / trusted host:
- 確認用LINEアカウントA / B（秘密値・個人情報を書かない）:
- 個別チャット / 拡大表示 / 下書き受信箱の既定OFF・同期:
- OFF request status / A着信数 / FAQ増加数:
- ON request status / A着信数 / FAQ増加数:
- FAQ質問・回答一致 / 無効表示 / 編集導線 / profile情報自動付加数:
- BでのA由来FAQ件数 / Aでの再取得一致:
- approve POST数 / A・B LINE着信数 / その他送信数:
- fail-safe host強制試験（実施または対象外理由）:
- migration 143 / 既存ルール件数・内容の不変:
- 保存前後のreplyActions ordered array:
- text返信時刻 / 4アクション反映時刻・最終状態:
- 2回目のタグ重複数 / フィールド最終状態:
- postback実測（PASS / BLOCKEDと理由）:
- 失敗アクション / 後続成功 / 返信結果:
- 送信失敗fixture（PASS / BLOCKEDと理由）:
- friend未連携fixture（PASS / BLOCKEDと理由）:
- 未設定ルールの返信差分 / タグ・フィールド変更件数:
- silentルールの返信件数 / アクション件数:
- 想定外LINE送信数（0）/ 実顧客更新数（0）:
- 撤収結果:
- 結果（PASS / FAIL / BLOCKED）と理由:

---

# resend-account-settings — host closer 実測チェック（2026-07-24）

## 目的と安全境界

選択中のLINEアカウント専用Resendキーでドメイン登録と自分宛テスト送信ができ、別アカウントへ設定が混ざらないことをtrusted hostで確認する。lane内では実キー入力・DNS変更・外部メール送信を行わず、以下は査読済みrevisionを確認環境へ反映した後にhost closerが実施する。

- [ ] 査読済みrevisionをtrusted hostへ反映し、additive migration `142_email_sender_settings_resend_api_key.sql` が対象D1へ適用済みであることを確認する。lane branchを本番へ直接deployしない。
- [ ] ownerが使用を許可した確認用LINEアカウントAと、A専用のResendアカウント・ドメイン・受信可能な差出人アドレスだけを使う。実在顧客のメールアドレスや別ownerのドメインで代用しない。
- [ ] Resend APIキー全文をログ、Console、スクリーンショット、このファイルへ残さない。Networkで保存requestを見る場合も画面共有・保存を止め、確認後にDevToolsの記録を消す。
- [ ] テスト送信は画面に保存した差出人アドレス本人宛の1通だけに限定する。LINE配信・個別返信は行わず、LINE送信0件を維持する。
- [ ] 安全な確認用アカウント、実キー、DNS変更権限、受信箱のいずれかを用意できなければ`BLOCKED`とし、実運用顧客や第三者宛で続行しない。

## アカウント別キー・ドメイン・コピーを確認する

1. [ ] 左上の共通アカウント切替で確認用LINEアカウントAを選び、通知設定のメール設定を開く。画面内に独自のアカウント選択欄がなく、左上の選択だけが使われることを確認する。
2. [ ] 差出人アドレスと差出人名を保存する。Resendの無料登録を完了し、管理画面で`Full access`のAPIキーを発行して「Resend APIキー」へ貼り付け、保存する。
3. [ ] 保存直後に入力欄が空になり、キーは`********`だけで表示されることを確認する。再読込後もマスク表示が残り、API response、画面、通常ログに平文キーが出ていないことを確認する。
4. [ ] 左上でLINEアカウントBへ切り替え、Aの差出人・マスク・ドメイン情報が表示されないことを確認する。Bが個別キー未設定なら「未設定（共通キーを使用します）」と表示されることを確認してからAへ戻る。
5. [ ] Aで「ドメインを登録してDNS設定を表示」を押し、表示された各DNSレコードの「名前」「値」コピーボタンを順に押す。クリップボードの内容が隣の読取専用欄と1文字単位で一致することを、安全なローカルテキスト欄へ貼り付けて確認する。
6. [ ] A専用ドメインのDNSへ表示レコードを登録する。反映後に「認証状態を確認」を押し、状態が「認証済み」になることを確認する。

## 自分宛テスト送信とfallbackを確認する

1. [ ] Aで「自分宛にテスト送信」を1回だけ押す。NetworkのPOST bodyが選択中の`accountId`だけで、任意の`to`や別メールアドレスを送っていないことを確認する。
2. [ ] 画面に成功結果が表示され、保存済み差出人アドレスの受信箱へテストメールがちょうど1通届くことを確認する。送信元ドメインと受信時刻だけを記録し、メール本文・完全なアドレス・headerの秘密情報は残さない。
3. [ ] ownerが共通Resendキーを設定済みで、確認用アカウントBの差出人ドメインもその共通キーで利用できる場合だけ、Bの個別キー未設定のまま同じテストを1回行う。条件を満たさなければこのfallback実測だけ`BLOCKED（安全な共通キー用ドメインなし）`と記録し、AのキーをBへコピーしない。
4. [ ] Aへ戻るとAのマスク・ドメイン・認証状態が維持され、Bの結果が混ざっていないことを確認する。Networkでテスト送信以外のメール送信0件、LINE送信0件であることを確認する。
5. [ ] 実測専用キーを破棄する運用なら、最後に「保存済みAPIキーを削除」を押し、マスクが消えて共通キーfallback表示へ戻ることを確認してからResend側でもキーを失効する。継続利用キーなら削除せず、ownerの秘密管理手順へ戻す。
...
- 査読済みrevision / deploy revision / trusted host:
- migration 142適用確認:
- LINEアカウントA / B（秘密値・個人情報を書かない）:
- Aの保存直後・再読込後のマスク / Bへの分離:
- ドメイン登録status / DNSコピー一致 / 認証status:
- Aのテスト送信API status / 受信時刻 / 着信数:
- 共通キーfallback実測（PASS / BLOCKEDと理由）:
- APIキー平文露出数（0）/ 任意宛先指定数（0）:
- LINE送信数（0）/ 想定外メール送信数（0）:
- 撤収結果:
- 結果（PASS / FAIL / BLOCKED）と理由:
