...
以前の仕組みで友だち一人ずつに固定された古いメニューが残っていると、「全員のデフォルト」を変えても、その人の画面は古いままでした。今回の修正では再適用時にLINE側の個別固定を確認し、今の全員デフォルトと違う人だけ固定を外します。同じメニューの人と個別固定がない人には触れません。画面には、何もしなかった人数とは分けて「個別固定を解除 N」と表示します。
...
- [ ] 既定一致の人や per-user GET が 404（個別固定なし）の人を解除していないことを、代表サンプルで確認する。

## host closer 実測手順

1. [ ] 実施前に `messages_log` 件数、フォロー中人数、旧個別メニュー、新しい全員デフォルト、確認用サンプルの友だちIDを控える。
2. [ ] 管理画面で「既存の友だちへ再適用」を1回だけ実行し、完了まで待つ。
3. [ ] 完了画面の「個別固定を解除」「変更なし」「失敗」を記録する。旧個別メニュー保持者がいるのに解除0件、または失敗が残る場合は端末確認へ進まず FAIL とする。
4. [ ] サンプルを LINE API で再照会する。`GET /v2/bot/user/{userId}/richmenu` が 404（個別固定なし）になり、`GET /v2/bot/user/all/richmenu` が新しい既定 `richmenu-5e23df11ef7fd5c0963ae312d4a4d10d` と一致することを確認する。既定一致サンプルは解除対象になっていないことも確認する。
