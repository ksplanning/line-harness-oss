...
- [ ] 査読済み revision を trusted host の確認環境へ反映してから行う。sandbox から本番 API、DB、設定を直接変更しない。
- [ ] LINEへの送信は一切行わない。配信の「作成」「予約」「送信」、個別チャットの「送信」、AI下書きの「承認して送信」は押さない。
- [ ] channel access token、cookie、password、API key を画面記録、スクリーンショット、ログへ残さない。Network では URL、method、status、秘密値を除いた response の数値だけを確認する。
- [ ] 本番3フォームには触れない。フォームの閲覧、編集、公開、回答送信も行わない。
- [ ] 比較対象は同じ月・同じ LINE公式アカウントにそろえる。確認中に別担当者が配信すると使用数が動くため、その場合は時刻をそろえて再確認する。

## host 実測手順

1. [ ] LINE Official Account Manager で確認対象のアカウントを開き、当月の料金プラン、最大送信数、使用数を表示する。残りは「最大 − 使用」で計算し、確認時刻（JST）と数値だけを控える。
2. [ ] 同じアカウントを trusted host の管理画面で選び、`LINEアカウント管理` を開く。「LINE公式の送信数」に、プラン（推定）・最大・使用・残りの4項目が日本語で表示されることを確認する。
3. [ ] 手順2の最大・使用・残りが手順1の LINE Official Account Manager と一致することを確認する。プラン名には「相当（推定）」が付き、公式確定情報のように見えないことも確認する。
4. [ ] DevTools の Network で、ブラウザーが worker の `GET /api/line-accounts/<account-id>/quota` だけを呼び、response に channel access token が含まれないことを確認する。LINE向けの送信系 `POST` が発生していないことも確認する。
5. [ ] `一斉配信` で「新規配信を作成」を開き、「残り送信数 N通 (対象 M人)」が表示されることを確認する。対象を「全員」「タグで絞り込み」「複数アカ重複除外」と切り替え、M人が各画面の対象数に追従することを確認する。
6. [ ] 対象人数が残りを上回る組み合わせを選べる場合は、赤い「残り送信数が対象人数より少ないです」という警告が出ても「作成」操作自体は無効にならないことを確認する。ただし、実際には作成も送信もしない。該当する組み合わせが無ければ、その旨を記録し自動UIテストの証跡を併記する。
7. [ ] `個別チャット` で確認用の会話を1件開き、送信欄の近くに控えめな「残り N通」バッジが表示され、本文入力や送信ボタンを邪魔していないことを PC 幅と390px幅で確認する。文字入力や送信は行わない。
8. [ ] もう一方のテナントへ切り替え、そのテナントで管理する別 LINE アカウントについて手順1〜7を繰り返す。それぞれの数値が混ざらず、各アカウントの LINE Official Account Manager と一致することを確認する。
9. [ ] 5分以内に同じ画面を2回再読み込みし、表示が壊れず同じ値を返すことを確認する。host の worker log を安全に閲覧できる場合だけ、2本の LINE quota API が再読み込みごとに呼ばれていないことも確認する（秘密値は記録しない）。

## 実測結果の記入欄

- 実施日時（JST）:
- 査読済み revision / deploy revision:
- 確認した tenant / LINEアカウント（秘密値を書かない）:
- LINE公式管理画面のプラン / 最大 / 使用 / 残り:
- アカウント管理4項目との一致:
- 一斉配信の残り / 対象人数 / 超過警告:
- 個別チャットの PC / 390px バッジ:
- アカウント切替後も数値が混ざらないこと:
- quota endpoint に token 非露出 / LINE送信系 POST 0件:
- 5分キャッシュ確認:
- 本番3フォーム不接触 / LINE送信0件:
