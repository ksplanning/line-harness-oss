...
- [ ] 査読済み revision を trusted host の確認環境へ反映し、この確認専用の自前配信フォームを新しく1件作る。
- [ ] **本番で使っている3フォームには触らない。** 既存フォームの保存・公開状態変更・項目追加や削除も行わない。
- [ ] 架空の住所だけを使い、token、key、password、回答者情報を画面記録へ残さない。
- [ ] sandbox から本番 API、DB、Google Sheets、migration を直接操作しない。管理画面と回答者向け公開ページからだけ確認する。
- [ ] 検証用フォームへ「郵便番号」「住所」「都道府県」「市区町村」「町名・番地」を追加する。

## host 実測手順

1. [ ] 「住所」パーツがパレットにあり、追加すると狭い1行欄ではなく、最初から最低2行相当の高さがある折り返し入力欄になることを確認する。
2. [ ] 「郵便番号」を選び、「郵便番号から住所を自動入力する」をONにする。「分割」「一括」という2つの選択肢が表示され、最初は従来どおり「分割」が選ばれることを確認する。
3. [ ] 「一括」を選ぶ。分割用の3つの入力先が隠れ、「住所の入力先」だけが表示されることを確認し、手順1の「住所」を選んで保存する。再読み込み後も「一括」と入力先が残ることを確認する。
4. [ ] 管理画面のプレビューで郵便番号 `１００－０００１` を入力し、「郵便番号から住所を入力」を実際にクリックする。「住所」欄へ都道府県・市区町村・町域が空白なしで連結して入ることを確認する。
5. [ ] 自動入力された住所の後ろへ、架空の番地と建物名を追記する。もう一度検索しても追記が消えないことを確認する。
6. [ ] 「住所」欄の途中で Enter を押す。改行もフォーム送信も起きないことを確認する。続けて `1行目`、改行、`2行目` の文字列を貼り付け、改行部分が半角スペースになって `1行目 2行目` と入ることを確認する。
7. [ ] 画面幅を390px程度まで狭くし、長い住所が欄の中で2行以上に自動折り返しされることを確認する。この折り返しは表示だけで、入力値に改行がないこともブラウザーの入力値で確認する。
8. [ ] 検証用フォームを公開し、回答者向けページで手順4〜7をもう一度行う。プレビューと同じく、一括入力・追記保持・Enter無効・貼り付け改行のスペース化・見た目の折り返しが動くことを確認する。
9. [ ] 公開ページから架空の回答を1件送る。管理画面の保存回答と、検証用Sheet連携を設定している場合はそのセルも確認し、「住所」が改行を含まない1行データで渡っていることを確認する。
10. [ ] 郵便番号設定を「分割」へ戻し、都道府県・市区町村・町名・番地の3つの入力先が再表示されることを確認する。プレビューと公開ページで、従来どおり3項目へ別々に入ることを確認する。
11. [ ] 確認後は検証用フォームだけを非公開化または削除し、本番3フォームを変更していないことを再確認する。
...
- 「一括」の保存・再読み込み / 実クリックでの連結住所:
- 追記保持 / Enter無効 / 貼り付け改行のスペース化:
- 390pxでの折り返し表示 / 入力値が1行であること:
- 公開ページがプレビューと同じだったか:
- 保存回答 / Sheetセルに改行がなかったか:
- 「分割」の従来動作:
- 本番3フォームへ触れていないこと:
