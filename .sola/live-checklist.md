...
1. [ ] deployment SHA、tenant、LINE account、テスト用 rule ID、`response_type`、automation 使用有無、リッチメニューの action type を記録する。
...
3. [ ] Piecemaker で既に確認済みの `source=auto_reply_keyword`・未読+0・未対応+0 と比較する。KS だけ結果が違う場合は、rule match、応答経路、保存 marker、migration/deploy SHA のどこが違うかを記録し、推測だけで PASS にしない。
