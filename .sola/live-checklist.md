...
ふだんのキーワード自動返信は、返信が本当に届いたときだけ「未読」と「未対応」の両方へ積まれません。人が確認する必要のある用件だけ、例外ルールの「未対応リストに残す」をオンにすると、自動返信後も未対応へ残ります。自動返信が失敗したもの、停止中のルール、普通のメッセージは、見落とさないよう今までどおり未読・未対応の両方へ残ります。過去の未対応データは勝手に変更しません。
...
- [ ] 既に成功済みの Piecemaker の直接返信結果だけで代用せず、owner 報告と同じ KS の tenant・LINE account・例外ルール経路をテスト用設定で再現する。ルールが直接返信か `silent` + automation かを記録する。
...
3. [ ] 自動返信が実際に端末へ届いたことを確認する。届かなければこの項目を PASS にせず、失敗時の手順へ進む。
4. [ ] 対象チャットの未読数が増えず、未対応バッジ・未対応一覧も増えないことを確認する。受信行は成功 marker、`silent` 経路なら automation の実送信成功後にだけ成功 marker へ変わっていることも確認する。

### 2. 人の確認が必要なルール — 未対応だけへ残す

1. [ ] 同じテストルールの「未対応リストに残す」をオンにして保存する。一覧表示が「残す」であることを確認する。
2. [ ] 未読数と未対応数の開始値を記録し、登録キーワードを1回だけ送る。
3. [ ] 自動返信が実際に届き、未読数は増えず、未対応数だけが1件増えて対象チャットが未対応一覧へ出ることを確認する。受信行には opt-in 専用 marker が保存されていることも確認する。

### 3. 自動返信失敗またはルール停止 — 両方へ残す
...
- KS rule ID / response type（直接返信 または silent + automation）/ automation ID:
