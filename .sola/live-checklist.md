...
画面のテスト受信者を間違えて増やしても、サーバーが別に持つ許可リストに入っていない人へは送れません。成功時は「何件送ったか」だけでなく、実際に送った userId を画面と送信結果へ出します。sandbox では LINE へ送らず、査読後の trusted host で、あやこへの1回だけの成功と、隔離 fixture への送信前拒否を確認します。

## host closer の実行条件と安全境界

- 査読済み revision を trusted host へ反映した後、owner または指定 closer だけが実施する。sandbox から実 LINE 送信、deploy、本番 D1 更新は行わない。
- 成功実測の対象は「あやこ」(`U5217ceb4debd9849959446ce8f902a27`) 1人だけとする。画面の `test_recipients` とサーバー許可リストの解決値が、ともにこの1人だけであることを開始前に確認する。
- LINE 実射は、あやこへのテスト送信1回だけ。timeout、通信断、結果不明では再送せず `BLOCKED` とする。
- 未登録宛先の拒否実測には、実在の友だちを使わない。trusted host 上の隔離 DB と outbound capture を使い、同一 account の synthetic fixture を設定する。安全な capture を用意できなければ `BLOCKED` とし、実友だちで代用しない。
...
2. 設定画面のテスト受信者があやこ1人だけで、受信者 preview にもあやこ1人だけが出ることを確認する。別 userId が出たら送らず `BLOCKED` とする。
3. 個人情報を含まない一意な marker を本文にしたテスト送信を **1回だけ**実行する。ボタンを再クリックせず、完了まで待つ。
4. あやこの LINE に1通だけ届いたことを確認する。API 応答と画面には `sent=1` と実際の `sentUserIds=["U5217ceb4debd9849959446ce8f902a27"]` が表示され、匿名の `sent:1` だけの結果になっていないことを確認する。
5. 同じ request の保存済み `response_json` にも同じ userId があり、test outgoing の増分が1件だけで、他 userId への増分が0件であることを確認する。

## 2. 未登録宛先の拒否を送信前に実測

1. trusted host の隔離 DB に、対象 account と同じ accountId を持つ synthetic friend と、その friend だけを指す `test_recipients` fixture を作る。fixture の LINE userId はサーバー許可リストへ追加しない。
...
- [ ] あやこ宛の実射は1回だけで、LINE受信1件、test outgoing 1件、他 userId への送信0件だった。
- [ ] 成功 API・画面・保存済み結果に、あやこの実 userId が明示され、匿名件数表示がなかった。
...
- [ ] owner確認: 「画面で宛先を間違えても、サーバー未許可の人へは送れず、送れた相手は userId で分かる」を実測できた。
