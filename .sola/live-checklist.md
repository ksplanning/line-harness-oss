...
- [ ] 査読済み revision の Worker / Web を preview または許可済み検証環境へ反映してから始める。
- [ ] 本番 3 フォーム、既存フォーム、Formaloo backend、LINE 送信には触れない。その場で作る使い捨て internal フォーム 1 件だけを使う。
- [ ] token、key、password、顧客画像を記録へ貼らない。試験画像は 1.5〜3MB 程度、10MB 未満の架空 PNG を使う。
- [ ] 作成した form ID と、`media/form-image/{formId}/` で始まる試験 R2 key だけを撤収対象として控える。

## 1. 3 つの画像置き場所を保存

- [ ] 高機能フォームを新規作成し、配信方式を「自前フォーム」に切り替える。
- [ ] デザインの「ロゴ」に試験 PNG を設定して保存する。画面を再読込してもロゴが残り、保存エラーが出ないことを確認する。
- [ ] デザインの「背景画像（全面）」にも同じ条件の試験 PNG を設定して保存する。再読込しても背景が残ることを確認する。
- [ ] 「装飾 ＞ 画像」をフォーム先頭へ置き、幅を「全幅」にして試験 PNG を設定する。保存後に再読込しても画像が残ることを確認する。
- [ ] ブラウザの Network または許可済み診断で、確定 URL が `/images/media/form-image/{formId}/...` になり、`data:image` のまま保存されていないことを確認する。

## 2. 公開ページで実表示

- [ ] 使い捨てフォームを公開し、発行された `/f/{formId}` を新しいブラウザセッションで開く。
- [ ] フォーム先頭の装飾画像が実際に表示され、壊れた画像や 500 にならないことを確認する。
- [ ] 画像リクエストが 200、画像の Content-Type、表示サイズが期待どおりであることを確認する。
- [ ] 参考として、管理画面の再読込後もロゴと背景の確定画像がプレビューへ戻ることを確認する。

## 3. 正直エラーと撤収

- [ ] 許可された preview で R2 失敗を安全に注入できる場合だけ、保存が 4xx と「画像の保存に失敗しました (サイズ/形式)」を返し、保存前の画像が維持されることを確認する。注入できない環境では自動テスト 3 ケースの証跡で代替する。
- [ ] 使い捨てフォームを非公開化して削除する。
- [ ] 控えた form ID と完全一致する `media/form-image/{formId}/` 配下の試験 object だけを、承認済み host 手順で削除する。他 prefix は削除しない。
- [ ] 公開 URL が利用不能になり、試験 object が残っていないことを確認する。

## 実施記録

- 実施日時: 未実施（査読後に host closer が記入）
- 実装 revision: `7e2d484` 以降の査読済み HEAD
- 環境: 未記入
- 実施者: 未記入
- 結果: 未実施
- 作成 form ID / 削除確認: 秘密値を含めず host closer が記入
...
- [ ] 対象 LINE アカウントで、時刻入りの重複しない確認用キーワードを決める。
- [ ] 確認専用テンプレートパックを作り、テキストと Flex を含む合計5吹き出しを順番どおり登録する。
- [ ] 自動返信ルールで「テンプレートパックから」を選び、5吹き出しへ展開する。Flex は「ビジュアルで編集」を開き、JSON直書きなしで保存する。
- [ ] ルール保存後にもう一度編集し、5吹き出しの種類・内容・順序が変わらず読み込まれることを確認する。
- [ ] 6吹き出しになる操作は保存・展開できず、「最大5件」の理由が画面に表示されることを確認する。
- [ ] FAQ設定で「下書きにする」「自動で送信する」をクリックし、確定後に選んだ方だけが緑色になり、設定の動き自体は従来どおりであることを確認する。

## あやこへ1回だけ実射

- [ ] あやこ本人の LINE user ID が `U5217ceb4debd9849959446ce8f902a27` と一致することを再確認する。
- [ ] あやこから確認用キーワードを1回だけ送る。
- [ ] 1回の応答で吹き出しがちょうど5つ、保存した順番どおり届き、Flexも崩れていないことを確認する。再送はしない。

## 撤収

- [ ] 確認専用の自動返信ルールとテンプレートパックを削除する。
- [ ] 確認用キーワードに応答しない状態へ戻ったことを管理画面で確認する。追加の実射はしない。
- [ ] 実施日時、査読済み revision、結果、削除完了をこの節へ記録する。秘密値や本文の個人情報は書かない。
...
- [ ] 実アカウントで1回目の取込が完了し、「新規 N 名 / 既存 M 名 / 失敗 K 名」の実数を記録した。
- [ ] 友だち総数の実増分が新規N名と一致した。
- [ ] 一斉配信は送信せず、画面の宛先カウント増加だけで既存配信対象に含まれることを確認した。
- [ ] 取込は1回だけ実行し、取込前からいた友だちの情報が変わらなかった。
- [ ] LINE送信0件、テスト送信0件、本番3フォーム・Formaloo・Discordへの接触0件、秘密値と個人情報の証跡記録0件だった。
...
- [ ] 回答1回目: 左に友だち台帳、右に回答が同じ1行で表示され、検証用友だちの行数は1だった。
- [ ] 列挿入後: 自社列と値を保ったまま見出し名で再同期できた。
- [ ] 再回答: 同じ行が更新され、追加行0、重複0だった。
- [ ] シート編集を実施した場合: W4aと同じ経路で最新回答だけが更新され、新規回答行0だった（未実施なら理由を `BLOCKED` と記録）。
- [ ] 見出し変更を実施した場合: 列の自動再作成0、警告1件、見出しを元へ復元済み（未実施なら理由を `BLOCKED` と記録）。
- [ ] 撤収後: 検証用データ0、本番3フォームへの接触0、LINE送信0、Discord投稿0、秘密値・個人情報の証跡記録0だった。
