...
4. [ ] 回答本文を確認して「確認して承認」を押し、その行が一覧からすぐ消え、件数が1件減ることを確認する。
5. [ ] 既定表示へ戻すと回答自体と直前の差分履歴が残り、回答本文・送信日時・添付が承認前と同じであることを確認する。回答シートとLINEには書き込み・送信が発生していないことも確認する。
6. [ ] 同じ回答を編集URLからもう一度変更し、再び未承認一覧へ現れ、経路が「編集URL」、差分が今回の変更前後へ置き換わり、承認状態が未承認へ戻っていることを確認する。
...
- [ ] desktop と 375 × 812 の両方で、件数付きボタン、経路表示、差分、承認ボタンが読め、承認後に空表示または次の行へ自然に切り替わることを確認する。
- [ ] desktop の統計カードと列見出し、375px の行ラベルが「LINE連携」、状態値が「連携済み / 未連携」であり、「確認状況 / 確認済み / 未確認」がこの画面に残っていないことを確認する。
