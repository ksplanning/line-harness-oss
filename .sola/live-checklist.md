# sheets-sync-dispatch-selfcall-fix — host SELF継続チェック

## オーナー向けの説明

1,450件の架空データを200件以下ずつ同期し、Cloudflare上でWorkerが自分自身を次の実行として呼び出せることを確かめます。途中で1回止めても0件からやり直さず、保存済みの続きから1,450件まで完走し、同じ友だちの行を二重に作らないことが合格条件です。
...
- [ ] 共有先はサービスアカウント `line-harness-sheets@line-harness-503015.iam.gserviceaccount.com` だけにする。鍵、token、cookie、passwordは表示・記録しない。
...
- [ ] 検証用接続だけを安全に1回中断する方法が用意できない場合は `BLOCKED` とし、本番cron、実運用設定、シート行の手削除で代用しない。

## 事前準備

1. [ ] owner提供の使い捨てスプレッドシートに空タブ `friends-1450-self-binding` を作り、上記サービスアカウントを編集者として共有する。
2. [ ] 検証専用LINEアカウントに、重複しない架空userIdの友だち1,450件を用意する。既存の実データへ追加しない。
3. [ ] 管理画面で検証用Sheets接続を新規作成し、対象URLとタブ名が使い捨てシートだけを指すことを二人確認またはスクリーンショットで確認する。
4. [ ] deploy revisionに、KSの `SELF → line-harness-ks` Service Bindingがあることを確認する。Piecemakerを検証する場合は `SELF → line-harness-piecemaker` も同様に確認する。
5. [ ] 開始前の検証用タブが空で、進行中の検証用同期jobが0件であることを確認する。

## host実測手順

1. [ ] 検証用接続の「今すぐ同期」を1回だけ押し、開始表示が `処理済み 0 / 1450件` になることを確認する。
2. [ ] 画面を再読み込みせず観察し、処理済み件数が1回につき200件以下で増えることを確認する。Cloudflareログでは `/internal/sheets-sync-work` がSELF Service Binding経由の別invocationとして続き、public URL自己fetchの1042系エラーが無いことを秘密値なしで記録する。
3. [ ] 1チャンク以上が完了した後、host担当者が用意した検証用の一回限りの方法で次の継続だけを中断する。検証用接続以外の設定や本番cronは変更しない。
4. [ ] 中断後も、例として `処理済み 400 / 1450件` のように最後に保存済みの件数が残り、0件へ戻らないことを確認する。エラー表示にGoogle応答本文や秘密値が出ないことも確認する。
5. [ ] 同じ検証用接続で「続きから再開」を1回だけ押し、中断前の件数より先へ200件以下ずつ自動継続することを確認する。
6. [ ] `処理済み 1450 / 1450件` と完了表示を確認する。使い捨てタブが見出し1行＋データ1,450行で、架空userIdの重複が0件であることを確認する。
7. [ ] 同じ接続でもう1回「今すぐ同期」を完走させ、データ行が1,450行のまま、同じuserIdの二重書込が0件であることを確認する。
8. [ ] 5分cron経路でも検証用接続だけを1回対象にし、同じSELF継続で完了または変更なしの冪等結果になることを確認する。cronを手動HMAC public self-fetchで代用しない。
9. [ ] 検証用接続を無効化・削除し、owner提供の使い捨てスプレッドシートを破棄する。実運用シート・本番3フォームの変更0件、LINE送信0件を記録する。
...
- trusted host / tenant:
- owner提供の使い捨てシート名・タブ名（URLや秘密値を書かない）:
- SELF binding（binding名 / service名）:
- 進捗表示の推移（各増分が200件以下）:
- 中断時の保存済み件数 / 表示された安全なエラー:
- 再開後に0へ戻らなかったこと:
- 1,450件完走 / データ行数 / userId重複数:
- 2回目完走後のデータ行数 / 二重書込数:
- cron経路の結果 / public self-fetch代用なし:
