...
- D-2: PASS — `rtk pnpm --filter web exec vitest run --config vitest.config.ts src/app/chats/chat-inline-draft.test.tsx src/app/chats/chat-history-popup.test.tsx` → 履歴の flex 主役構造、1行 header、通常送信・メモ保存・対応状態・拡大表示の回帰を含む 2 files / 30 tests passed (exit 0)
