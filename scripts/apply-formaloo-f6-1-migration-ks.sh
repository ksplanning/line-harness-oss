#!/usr/bin/env bash
# apply-formaloo-f6-1-migration-ks.sh — migration 094 (formaloo_workspaces / envelope 暗号化
# キー管理台帳 / F6-1 S-1) を本番 D1 に安全適用する。additive のみ (CREATE TABLE/INDEX IF NOT
# EXISTS) で既存行・既存認証には無影響。dark-ship (KEK 未投入時は resolver が null 経由 env
# fallback = byte-equivalent / registry は空のまま)。
set -euo pipefail

DB="${DB_NAME:-line-harness-ks}"
REPO_ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
MIG_DIR="$REPO_ROOT/packages/db/migrations"
MIGRATIONS=(094_formaloo_workspaces.sql)
...
echo "==> [2/5] migration 094 を適用 (benign 'duplicate column'/'already exists' は許容)"
...
echo "==> [3/5] _migrations 台帳に 094 を記録"
for m in "${MIGRATIONS[@]}"; do
...
echo "==> [5/5] formaloo_workspaces 存在 + pragma_table_info verify (平文鍵列 0 件)"
WR d1 execute "$DB" --remote --command \
  "SELECT name FROM sqlite_master WHERE type='table' AND name='formaloo_workspaces'" \
  | grep -q "formaloo_workspaces" && echo "    table present" \
  || { echo "!!! formaloo_workspaces が見えない" >&2; exit 1; }
WR d1 execute "$DB" --remote --command \
  "SELECT name, type FROM pragma_table_info('formaloo_workspaces')"

echo "DONE: migration 094 (F6-1 formaloo_workspaces) を本番 D1 ($DB) に安全適用しました。"
