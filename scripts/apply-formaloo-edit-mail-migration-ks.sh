#!/usr/bin/env bash
# apply-formaloo-edit-mail-migration-ks.sh — migration 101 (formaloo_forms.allow_edit_mail /
# edit_mail_field_slug / edit_link_epoch + formaloo_edit_mail_sends outbox 表 / 弾L Phase A) を
# 本番 D1 に安全適用する。additive のみ。両テナント適用は closer が dual (このスクリプトは KS 用)。
set -euo pipefail

DB="${DB_NAME:-line-harness-ks}"
REPO_ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
MIG_DIR="$REPO_ROOT/packages/db/migrations"
MIGRATIONS=(101_formaloo_edit_mail.sql)
...
echo "==> [1/5] migration 101 を適用 (benign 'duplicate column'/'already exists' は許容)"
...
echo "==> [2/5] _migrations 台帳に 101 を記録"
for m in "${MIGRATIONS[@]}"; do
...
echo "==> [4/5] allow_edit_mail / edit_mail_field_slug / edit_link_epoch 列 + formaloo_edit_mail_sends 表 verify"
WR d1 execute "$DB" --remote --command \
  "SELECT name FROM pragma_table_info('formaloo_forms')" | tee /tmp/101-formaloo_forms-pragma.txt
for col in allow_edit_mail edit_mail_field_slug edit_link_epoch; do
  grep -q "\"name\": \"$col\"" /tmp/101-formaloo_forms-pragma.txt && echo "    column $col present" \
    || { echo "!!! $col 列が見えない" >&2; exit 1; }
done
WR d1 execute "$DB" --remote --command \
  "SELECT name FROM sqlite_master WHERE type='table' AND name='formaloo_edit_mail_sends'" | tee /tmp/101-table.txt
grep -q "formaloo_edit_mail_sends" /tmp/101-table.txt && echo "    table formaloo_edit_mail_sends present" \
  || { echo "!!! formaloo_edit_mail_sends 表が見えない" >&2; exit 1; }

echo "==> [5/5] 既存行の allow_edit_mail が既定 0 verify (NOT NULL DEFAULT 0)"
WR d1 execute "$DB" --remote --command \
  "SELECT COUNT(*) FROM formaloo_forms WHERE allow_edit_mail IS NULL" | grep -qF '"COUNT(*)": 0' \
  && echo "    all rows allow_edit_mail NOT NULL (既定 0)" \
  || { echo "!!! allow_edit_mail に NULL 行あり = 想定外" >&2; exit 1; }

echo "DONE: migration 101 (formaloo_forms.allow_edit_mail 等 + formaloo_edit_mail_sends / 弾L Phase A) を本番 D1 ($DB) に安全適用しました。"
